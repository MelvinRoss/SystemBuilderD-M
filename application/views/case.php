...
var coreInput,  // input tied to xSlider
    speedInput,  // input tied to ySlider
...
coreInput = Y.one( "#core_value" );
coreInput.setData( "slider", new Y.Slider({
            axis: 'x',
            min   : 1,      // min is the value at the top
            ...
            after : {
                valueChange: Y.bind( updateInput, coreInput )
            }
        }).render( ".core_slider" ) // render returns the Slider
    )                               // set( "data", ... ) returns the Node
    .on( "keyup", updateSlider );   // chain the keyup subscription
    
    // Create the ram Slider.
speedInput = Y.one( "#speed_value" );
speedInput.setData( "slider", new Y.Slider({
            axis: 'x',
            min   : 1,      // min is the value at the top
            max   : 4.1,     // max is the value at the bottom
            value : 3,       // initial value
            length: '201px',  // rail extended to afford all values

            // construction-time event subscription
            after : {
                valueChange: Y.bind( updateInput, speedInput )
            }
        }).render( ".speed_slider" ) // render returns the Slider
    )                               // set( "data", ... ) returns the Node
    .on( "keyup", updateSlider );   // chain the keyup subscription
</script>
    -->
...
echo "<body>
  <div>";
     ...
     <fieldset><div id="legend">Case</div>
    <div id="introborder">
        <div id="inborder">
<?php
       ...
       $this->load->view('partforms/caseform'); 
            echo "</div>";
        }
        else
        {
            echo "<div id='searchpart'>";
            $this->load->view('partsearch/searchcase');
            echo "</div>";
        }
?>
